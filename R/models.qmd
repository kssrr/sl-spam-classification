...
Below are the model specifications and tuning grids I use (regular grids):
...
Glueing together & tuning. I bundle the preprocessing recipe into [workflows](https://workflows.tidymodels.org/) together with the models & then tune them using regular grid search:
...
Selecting the best competitor models by precision, fitting to train & collecting their metrics on the training set:

```{r}
train_fits <- 
  models |> 
  mutate(
    last_fit = map2(workflow, tuning_res, function(wf, tune) {
      wf |> 
        finalize_workflow(select_best(tune, metric = "precision")) |> 
        fit(train)
    }),
    metrics_train = map(last_fit, function(mod) {
      mod |> augment(new_data = train) |> class_metrics(truth = spam, estimate = .pred_class)
    })
  )
```

Comparing their performance to that of the neural network:
...
train_fits |> 
  select(name, metrics_train) |> 
  unnest(metrics_train) |> 
  pivot_wider(names_from = ".metric", values_from = ".estimate") |> 
  select(name, precision, accuracy, f_meas) |> 
